...
// Month represents a Month object. Compare Month objects using a.month ===
// b.month or a.equal(b);
class Month {
  constructor(i) {
...
class Time {
  ...
  addDate(years, months, days) {
    ...
    return time.date(years+dateResults.year,
...
time.date = function(year, month, day, hour, min, sec, nsec, loc) {
  ...
  d = d.add(Uint64.from(daysBefore[month.month-1]));
...
var _parse = function(layout, value, defaultLocation, local) {
  ...
  if (day < 1 || day > time._daysIn((new time.Month(month)), year)) {
    throw new ParseError(alayout, avalue, "", value, ": day out of range");
  }

  if (z !== null) {
    return time.date(year, Month(month), day, hour, min, sec, nsec, z); // TODO
  }

  if (zoneOffset !== -1) {
    var t = time.date(year, new time.Month(month), day, hour, min, sec, nsec, location.UTC);
  ...
  return time.date(year, new time.Month(month), day, hour, min, sec, nsec, defaultLocation);
};
...
time.parse = function(layout, value) {
  internal.areStrings([layout, value]);
